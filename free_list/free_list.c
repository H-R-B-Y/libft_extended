...
/*   Updated: 2025/11/03 19:09:17 by hbreeze          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_returncode	init_free_list(t_free_list *free_list, size_t size)
{
	...
	((t_free_list_node *)free_list->arena)->content
		= 0 + sizeof(t_free_list_node);
	((t_free_list_node *)free_list->arena)->next
		= FREE_LIST_NULL_OFFSET;
	((t_free_list_node *)free_list->arena)->size
		= size - sizeof(t_free_list_node);
	return (RETURN_OK);
}
